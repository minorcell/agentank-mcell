...
function onIdle(me, enemy, game) {
  ...
  // ===== 1. BULLET DODGE (highest priority) =====
  ...
  if (enemyTank) {
    ...
    if (clearShot) {
      ...
      // FIXED: TURN FIRST, then freeze+fire
      // This ensures fire lands while enemy is frozen
      if (myDir !== aimDir) {
        me.turn(turnDirection(myDir, aimDir));
        return;
      }

      // Now facing enemy - freeze then fire (freeze covers this frame and next)
    ...
    // Freeze at close range to lock enemy while we get into position
    if (freezeReady && dist <= 5 && !me.bullet) {
      me.freeze();
      return;
    }

    // Navigate toward enemy (BFS only, no greedy wall-hugging)
...
// BFS shortest path - returns first step from start toward goal
