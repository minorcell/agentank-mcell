// Persistent memory across onIdle calls
var _lastEnemyPos = null;
var _lastEnemySeenAt = -100;
...
function onIdle(me, enemy, game) {
  ...
  // Track enemy position for hunting after losing sight
  if (enemyTank) {
    _lastEnemyPos = enemyTank.position;
    _lastEnemySeenAt = game.frames;
  }
  ...
  if (enemyBullet && isBulletComing(myPos, enemyBullet.position, enemyBullet.direction)) {
    ...
    // Fallback: move forward (away from bullet path)
    var fwd = add(myPos, delta(myDir));
    if (isOpen(fwd, map)) {
      me.go();
      return;
    }
    // Last resort: turn and try
    me.turn("right");
    return;
  }
  ...
  if (enemyTank) {
    ...
    // If close and aligned but blocked by terrain, shoot anyway
    if (aligned && canShoot(myPos, ePos, map)) {
      ...
      // Freeze + Fire combo
      if (!me.bullet && me.skill.remainingCooldownFrames === 0) {
        me.freeze();
        return;
      }
      if (!me.bullet) {
    ...
    // If close enough, try to freeze even if not aligned
    // This locks them down while we maneuver into position
    if (dist <= 4 && me.skill.remainingCooldownFrames === 0 && !me.bullet) {
      // Only freeze if they're in a direction we can turn toward in 1-2 frames
  ...
  if (!enemyTank && _lastEnemyPos && game.frames - _lastEnemySeenAt < 30) {
    var step = nextStep(myPos, _lastEnemyPos, map);
    if (step) {
      moveToward(me, myDir, myPos, step);
      return;
    }
  ...
  // ===== PRIORITY 5: EXPLORE (center of map) =====
