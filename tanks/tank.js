...
function onIdle(me, enemy, game) {
  ...
  if (enemyTank && !_dodging) {
    ...
    if (clearShot) {
      ...
      if (freezeReady && dist <= 3 && !me.bullet) {
        me.freeze();
        return;
      }
      ...
      // Bullet in flight: close distance to fire again sooner
      if (dist > 2) {
        var fwd = add(myPos, delta(aimDir));
        if (isOpen(fwd, map)) {
          if (myDir === aimDir) { me.go(); return; }
          me.turn(turnDirection(myDir, aimDir));
          return;
        }
      }
      // Very close or blocked ahead - strafe to dodge return fire
      var perp = (aimDir === "up" || aimDir === "down") ? ["left", "right"] : ["up", "down"];
    ...
    // Not aligned - if star is closer than enemy, grab it first
    if (game.star) {
      ...
      if (starDist <= dist) {
        ...
        greedyMove(me, myPos, myDir, game.star, map);
        return;
      }
    }
